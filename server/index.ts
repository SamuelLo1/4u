...
// Personality + 10 products using OpenAI (gpt-4o) from Q/A pairs with tags
app.post('/api/personality-products', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const system = `You are an interior stylist and product curator for bedroom setups. Given user Q&A pairs and tags, infer a concise personality and propose exactly 10 purchasable bedroom product ideas.
Return STRICT JSON only matching this schema:
{
  ...
  "products": [{"name":"string","searchQuery":"string","category":"BED|DESK|LAMP|RUG|WALL_ART|PLANT|STORAGE|DECOR|CHAIR|BEDDING","styleHints":["string"],"colorHints":["string"],"rationale":"string"}]
    ...
    const products = (data.products as any[]).slice(0, 10).map(p => ({
      name: String(p?.name || ''),
    ...
    while (products.length < 10) products.push({name: 'nightstand lamp', searchQuery: 'nightstand lamp', category: 'LAMP', styleHints: [], colorHints: [], rationale: ''})
